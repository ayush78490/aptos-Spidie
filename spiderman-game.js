...
SpidermanGame.prototype.load = function() {
    ...
    if (window.ethereum) {
        ...
        this.web3 = new Web3('https://rpc.test2.btcs.network');
    }
    ...
    // Initialize spiderman here to ensure it's available in loadNext
...
SpiderMan.prototype.update = function() {
	if (this.keyIsDown(KEY.W) && !this.hasState("FALL")) {
		this.addState("JUMP");
	}
	if (this.keyIsDown(KEY.D)) {
		this.addState("RUNNING");
		this.runningDirection = DIRECTION.RIGHT;
	}
	if (this.keyIsDown(KEY.A)) {
		this.addState("RUNNING");
